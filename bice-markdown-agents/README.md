...
All responses are `Cache-Control: public, max-age=300`,
...
When enabled, a small deferred script (`assets/webmcp.js`) registers two
tools with the [WebMCP API](https://webmachinelearning.github.io/webmcp/)
(`navigator.modelContext.provideContext()`) so browser-embedded agents can
act on the site:

- `search_content` — searches published content via the WP REST API
  (`/wp/v2/search`), returning titles and URLs.
- `get_page_markdown` — fetches any same-origin page with
  `Accept: text/markdown`, reusing this plugin's negotiation for
  token-efficient reading. Cross-origin URLs are refused.

In browsers without WebMCP the script exits immediately — no console
noise, no behavior change for human visitors.
...
vendor/bin/phpunit        # 185 tests: parser, guards, converter, signals, discovery
